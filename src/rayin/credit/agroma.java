...
public class agroma extends javax.swing.JFrame {

    public agroma() {
        initComponents();
        setIconImage(Toolkit.getDefaultToolkit().getImage(getClass().getResource("icon.png")));
        try {
            connect.search_all_deals("AGROMA");
        } catch (Exception ex) {
            Logger.getLogger(agroma.class.getName()).log(Level.SEVERE, null, ex);
    ...
    private void foodTableMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_foodTableMouseClicked
        ...
        try {
            DefaultTableModel borrower = (DefaultTableModel) foodTable.getModel(); // BORROWER PANEL BORRWER DATA SHOWIG TABLE 
            int borrowerRow = foodTable.getSelectedRow();
            cusName.setText(foodTable.getValueAt(borrowerRow, 1).toString());

            newBorrower.setEnabled(true);
    ...
    private void searchBarKeyReleased(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_searchBarKeyReleased
        ...
        try {
            clearTable();
            connect.search_every_field(lenth, "AGROMA");
        } catch (Exception ex) {
            JOptionPane.showMessageDialog(null, "Something went wrong !");
